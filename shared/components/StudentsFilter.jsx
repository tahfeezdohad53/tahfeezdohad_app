...
function StudentsFilter({ handleFilterStudents, reset, readOnly = false }) {
  const { user } = useUser();
  const { handleChangeSearchParams, searchParams,pathname } = useFilter();
  const [value, setValue] = useState("");

  function guardClause(){
    if (user?.role !== "admin" && !pathname.includes("gurfah")) return false;
    if (searchParams.get("batch")) return false;
    return true;
  }
  useSetSearchParams({ paramsList: [{ key: "batch", value: "yaqoot_mardo" },{key:'classStatus',value:"all"}],guardClause });
  useSetSearchParams({paramsList:[{key:'classStatus',value:'all'}],guardClause:() => user?.role === 'teacher' ? true : false});
    ...
    <div className="w-full pb-5 lg:w-1/2 lg:mx-auto flex flex-col items-center  gap-5">
      <div className="w-full h-full relative">
        ...
        {value.length > 0 && (
          <button
            onClick={handleReset}
            className="p-2 rounded-md hover:bg-gray-50 hover:cursor-pointer duration-300 ease-in-out  absolute right-4 top-1/2 -translate-y-1/2"
          >
            <HiOutlineXMark />
          </button>
        )}
      ...
      {user?.role === "admin" && (
        <div className="text-xs px-3 mt-3 justify-center flex-wrap flex gap-3">
          <button
